...
let metallicAngle;
let mcr;
const generateMetallic = n => {
  console.log(n);
  metallicRatio = (n + Math.sqrt(n * n + 4)) / 2;
  mcr = 1 / (metallicRatio * metallicRatio);
  metallicAngle = mcr * 2 * Math.PI;
};
// generateMetallic(1);
var petalShapes = [];
...
petalRadius = setDefault(petalRadius, metallicRatio * growthFactor);
...
metallicNumber = setDefault(metallicNumber, 1);
petalTransparency = setDefault(petalTransparency, 0);
generateMetallic(metallicNumber);
...
var goldenFilter = (breadth, magnitude, spirals) =>
  metallicAngle / (breadth + mcr * magnitude + (mcr * spirals) % 1);
    ...
    for (let index of petalShapeChoices) {
      petalShapes.push(
        [
          ...
          [ellipse, 1, 1 / metallicRatio],
          [ellipse, 1 / metallicRatio, 1],
          [rect, 2, 2],
          [rect, 2 / metallicRatio, 2],
          [rect, 2, 2 / metallicRatio]
        ][index]
      );
    }
  }
...
var rotator = ({rotationAngle: phi}) =>
  ...
  goldenFilter(1, 0.005 * metallicRatio, spiralCount);
...
function createSpiral(gf = growthFactor, radius = petalRadius) {
  ...
  for (let n = 0; n < as; ++n) {
    ...
    //   abs(Math.sin(phi % (metallicAngle / colorFilter) + colorScaler)),
    //   phi);
  }
  petals = arr;
  updater();
...
function updater() {
  ...
  //   console.log((1 + (mcr * 0.005 * (animator(5) % 10) + mcr * 1 % 1)));
}
  ...
  for (let setting of parameters.keys()) {
    ...
    switch (setting) {
      ...
      case "background_transparency":
        ...
        break;
      case "metallic_number":
        metallicNumber = (scratch);
        break;
    }
  }
}
